...
<div class="container page-body">
    <div class="row">
        ...
        <div class="col-10 user-tab-content">
            ...
            @foreach ($dsMaChuaHoanThanh as $maHD)
                <?php
                    $hoaDon = App\HoaDon::find($maHD);
                ?>
                <div class="donhangmoi row">
                    ...
                    <table width="100%" class="table table-bordered table-customize table-responsive col-12">
                        <thead>
                            <tr>
                                <th>Điện thoại</th>
                                <th>Số lượng</th>
                    ...
                    <button class="prm-btn" onclick="HuyDonHang({{ $maHD }})">Hủy đơn hàng</button>
                </div>
            @endforeach

            @foreach ($dsMaDaHoanThanh as $maHD)
                <?php $hoaDon = App\HoaDon::find($maHD); ?>
                <div class="donhangmoi row">
                    <a href="taikhoan/ChiTietDonHang/{{ $maHD }}.html">Xem</a>
                    <table class="tb1 col-6">
                        <tr>
                            <th>Mã đơn hàng:</th>
                            <td>{{ $hoaDon->Ma_hoa_don }}</td>
            ...
            @if (session('thongBaoHuy'))
                <?php
                    echo '<script>alert("'. session('thongBaoHuy') .'")</script>';
                ?>
            @endif
        </div>
    </div>
...
@section('script')
    ...
    <script>
        ...
        {
            if(confirm('Bạn sẽ hủy đơn hàng có mã là: '+id))
            {
                window.location.href = 'taikhoan/HuyDonHang/'+id+'.html';
            }            
        }
    </script>
@endsection